...
## 3. Arcade Block-A Build Path (what 0x05A2CE does step by step)

### Function layout

The block-A builder spans arcade `0x05A098`–`0x05A258` (genesis current: `0x05A2AE`–`0x05A468`).

**Preamble (arcade `0x05A098`, genesis current `0x05A2AE`, 32 bytes):**
...
363C 00E8   MOVE.W #0x00E8, D3        ; D3 = 0xE8 (Y-base / initial attr byte)
...
`0x059F90`, landing at arcade `0x05A0B8` (genesis current `0x05A2CE`), which is **32 bytes
into the function, after the preamble**. This mid-function entry relies on `A0`, `D2`, `D3`
already being correctly set, which they were not in the producer context. This was a design
inconsistency in D-7 that was tolerated; Phase 2 must call the **full entry** at `0x05A098`.

**Function body (arcade `0x05A0B8`, genesis current `0x05A2CE`):**
...
The function then iterates through up to 13 sprite entries (clamped from `D4` if > 0x0C),
...
```
30FC 0000   MOVE.W #0x0000, (A0)+    ; word0 = 0 (attr, may be updated by outer loop)
30C3        MOVE.W D3, (A0)+          ; word1 = D3 (Y coordinate base)
0640 03CA   ADDI.W #0x03CA, D0       ; adjust D0 with Y-row offset
30C0        MOVE.W D0, (A0)+          ; word2 = D0 + 0x03CA (tile code + row)
...
**Function ends at arcade ~`0x05A258`, genesis current `0x05A468`:** `RTS` (4E75).

The function reads A5-relative state (animation counter, sprite indices, control flags)
and writes fully populated 4-word entries to block-A. With valid state, it produces
nonzero word1/word2/word3 values that the renderer can decode into visible sprites.
...
(`0x05A098` arcade / `0x05A2AE` current genesis). No pre-setup by caller is required
beyond `A5` pointing to workram (which is always the case in this context).

---

## 4. What Was Lost in Phase 1 (D-7 removal removed the call to 0x05A2CE)

**D-7 original bytes** at arcade `0x059F90`:
```
61 00 01 24  BSR.W +0x0124   ; calls arcade 0x05A0B8 (= genesis current 0x05A2CE)
...
- Removed both BSR calls — block-A and block-B builder calls were silenced
...
## 5. Minimal Required Logic (is 0x05A2CE call alone enough?)

**No.** Calling `0x05A2CE` (the mid-function entry) alone is NOT sufficient because:

1. `A0` is NOT set by `0x05A2CE`. The preamble at `0x05A2AE` sets `A0 = 0xE0FF11FE`.
   At the time of the producer's RTS, `A0 = 0xE0FF01DC` (past the B-block fill area).
   Calling `0x05A2CE` without the preamble would write sprites to the wrong address.

2. `D2` and `D3` are NOT initialized by `0x05A2CE`. The preamble sets `D2 = 0x0010`
   (tile column width) and `D3 = 0x00E8` (Y base). Without these, the tile and Y values
   in the written entries would be garbage.

**Yes.** Calling the FULL entry at `0x05A098` (arcade) / `0x05A2AE` (current genesis) IS
sufficient because:

1. The preamble sets `A0 = 0xE0FF11FE` unconditionally.
2. The preamble sets `D2 = 0x0010`, `D3 = 0x00E8` unconditionally.
3. The preamble loads `D0 = a5@(0x013A)` (animation counter from workram).
4. `A5` is always the workram base in this context.
5. The function ends with `RTS` — returns cleanly to the caller (producer).

**Additional requirement:** None. Calling `JSR 0x05A098` (arcade) immediately before the
producer's `RTS` is the complete Phase 2 change. No register pre-setup is needed.
...
```json
{
  "arcade_pc": "0x059F90",
  "original_bytes": "4e75",
  "replacement_bytes": "4eb90005a2b44e75",
  "note": "PHASE 2: JSR to block-A builder (arcade 0x05A098, genesis 0x05A2B4 after Phase 2 shift) then RTS. Target = 0x05A098 + 0x200 + 28(shift_after_self) = 0x05A2B4. delta: +6 (2->8 bytes)."
}
```
...
0005a2b4    target genesis address (4 bytes): block-A builder preamble
4e75        RTS (2 bytes): producer return
```

**Shift impact:** delta changes from `0` to `+6`. All arcade code at addresses > `0x059F90`
shifts by +6 bytes in the genesis ROM. The block-A builder target is adjusted accordingly
(embedded as the post-shift genesis address `0x05A2B4`).

**No other changes required.** No `.c` files, Makefile, or other spec files need modification.

### Address computation note for Cody

The embedded address `0x05A2B4` is a **hardcoded final genesis address**. It is computed as:
```
target_arcade(0x05A098) + 0x200 + shift_before_target_after_phase2(28) = 0x05A2B4
```
The shift_table_patcher does NOT auto-relocate replacement_bytes content (only original
ROM code via `fix_absolute_longs`). The address is stable because no future shift entries
are planned between `0x059F90` and `0x05A098` in arcade space. Verify post-build with
the static check in Section 7.
...
Also verify the builder preamble is at the target:
```python
with open('dist/Rastan_NNN.bin', 'rb') as f:
    f.seek(0x05A2B4)
    data = f.read(8)
...
Fail: callsite still contains `4e75` only, or target is garbage bytes.
...
```
EXPECT at renderer entry (second call, after producer has run):
...
**Fail:** Title text renders (CREDI/TILT still works from Phase 1) but no sprite tiles appear.

---

## 8. Final Conclusion

The block-A builder function (arcade entry `0x05A098`, current genesis `0x05A2AE`) is a
fully self-contained sprite descriptor producer. Its preamble initializes `A0 = 0xE0FF11FE`,
`D2`, and `D3` before the main loop writes 4-word sprite entries. The function reads animation
state from `A5`-relative workram locations and writes populated descriptor tuples to block-A.

Phase 1 removed the D-7 BSR call to the mid-function entry (`0x05A2CE`), leaving block-A
at zero. Phase 2 requires a single spec change: replace the identity `4e75` at the
`0x059F90` shift_replacements entry with `4eb90005a2b44e75` (JSR to full builder + RTS).
This restores the block-A builder call in the producer path with correct register setup
and correct block-A base address targeting.
