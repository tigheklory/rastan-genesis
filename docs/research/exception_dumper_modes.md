...
- mode `0`: default SGDK handlers
- mode `1`: custom compact text-only dumper
...
## Mode Values
- `0` = Default SGDK exception vectors and behavior
- `1` = Custom text-only dumper (no QR generation/rendering)
- `2` = Custom QR dumper
...
# Mode 1 (text-only custom dump)
make -C apps/rastan release RASTAN_EXCEPTION_DUMPER_MODE=1
