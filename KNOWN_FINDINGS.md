...
**Build 0215 FG source-progression evidence.** Build 0215 refines the Stage 1 gameplay FG_SRC helper after Build 0213/Exodus showed the first repeated foreground/floor at frame `429`. The translated arcade path already advances and rebuilds the relocated PC080SN source tables (`Genesis_WRAM 0x00FF1000`, `0x00FF1040`, `0x00FF1080`) via JSON-mapped `arcade_pc 0x0558C8 -> runtime_genesis_pc 0x055AC8` and `arcade_pc 0x055904 -> runtime_genesis_pc 0x055B04`, but pre-Build-0215 `genesistan_stage_fg_src_column` ignored those runtime tables and recomputed source from the folded destination column. At the 64-column/page boundary, the destination folds modulo the C-window while the arcade-owned source tables continue advancing, so the helper replayed earlier foreground/floor groups. Build 0215 changes only the gameplay FG helper to read the rebuilt pointer table at `Genesis_WRAM 0x00FF1040` and `colidx = a5@0x10CA & 3`, preserving the existing tall-FG staging/projection route. Scripted right-held MAME comparison shows Build 0213 and Build 0215 match pre-boundary at frames `429` and `1000` (`tall_hash=57C2A48D`), then diverge after progression (`frame 1800`: Build 0213 `tall_hash=FDB0232D`, Build 0215 `tall_hash=D8767E3A`). Screenshots confirm the initial repeated floor pattern is no longer simply replayed, though gray/wrong lower-block content remains a separate unresolved visual issue.
...
- **Status:** ACTIVE
- **Confidence:** CONFIRMED (offline reproduction over Build 0153 ROM + shipped LUT; generator source read; runtime taps; Build 0217/0218 cave-continuation descriptor/LUT evidence)
...
- **Source Documents:** docs/design/Andy_gameplay_scene_selection_analysis.md; states/traces/build_0154_gameplay_scene_selection/repro_lut_mismatch.py; docs/design/Cody_build0217_cave_map_tileset_fix.md; states/traces/build0217_cave_map_tileset_fix_20260719_204341/reduced_findings.md; states/traces/build0217_cave_map_tileset_fix_20260719_204341/build0216_cave_lut_coverage_before_patch.tsv; states/traces/build0217_cave_map_tileset_fix_20260719_204341/split_model_cave_lut_coverage_after_data_regen.tsv; states/traces/build0218_cave_map_tileset_release_20260719_210451/prebuild_pc080sn_coverage_budget.tsv; states/traces/build0218_cave_map_tileset_release_20260719_210451/build0218_artifact_hashes.txt; states/traces/build0218_cave_map_tileset_release_20260719_210451/cave_representative_cell_mapping.tsv
- **Related Issues:** OPEN-017
- **Related Findings:** KF-040
- **Last verified:** 2026-07-19 (Build 0218 cave-residency candidate produced; cave visual acceptance still user/manual)
...
- **Build 0217 cave-continuation evidence:** the same runtime Stage 1 descriptor table continues after the outdoor attr `0x0002` entries with attr `0x0003` cave/interior entries: descriptor entries `56..111` use source bases `0x00F91C` and `0x01011C`. Build 0216/initial Build 0217 excluded them twice: the generator stopped before attr `0x0003`, and the runtime strip gate `[0x0000D31C,0x0000FB1C)` excluded relocated cave sources `0x0000FB1C` and `0x0001031C`. Pre-fix LUT coverage was `1/404` and `0/414` for the two cave source blocks. A naive outdoor+cave merge is not valid because it exceeds the PC080SN residency budget (`1422` tiles vs `1164`); the supported boundary is producer-source-selected residency (outdoor attr `0x0002` scene fits at `962`, cave attr `0x0003` scene fits at `568`). Source implementation for split residency was staged and assembly-checked in `docs/design/Cody_build0217_cave_map_tileset_fix.md`, but the numbered Build 0217 ROM was produced before the generated-data gap was corrected and is **not** a validated cave-fix artifact. Do not overwrite/reuse Build 0217; the corrected source requires a separately authorized next numbered build.
- **Build 0218 cave-residency materialization:** Build 0218 preserves Build 0217 and releases the split-residency data in a numbered ROM: `dist/rastan-direct/rastan_direct_video_test_build_0218.bin`, SHA256 `30a84f86cc34e8dc9861f945138e7aafabe6f072b466fa6d161b8b0e8ed60a95`, size `1,586,184`, counter `218`, config `PC090OJ_MIRROR_RECORDS=256 RASTAN_GAMEPLAY_HUD_SPRITES=0`, opcode_replace count `216`, coverage `0x183408`. Build 0218 generated-data evidence shows complete cave source coverage (`0x00F91C 404/404`, `0x01011C 414/414`) and representative cave cells mapping to real LUT slots / Genesis cell words. Runtime automation did not reach the Genesis cave source family: original arcade with the same input envelope reached a cave entrance/drop visual by frame `900`, while Build 0218 remained on outdoor source `0x0000D31C` / tileset `1` through frame `12005`. Therefore this finding is verified as a source/residency-model correction, but cave visual acceptance is still **USER MUST VERIFY** and should not be claimed from Build 0218 automation.
...
## KF-068 — Arcade VBlank tail must not lower IPM before final RTE after Genesis VBlank service growth; Build 0216 fixes the hurry-up swarm reentrant-IRQ stack/control corruption

- **Status:** ACTIVE
- **Confidence:** CONFIRMED (Build 0215 native traces show repeated IRQ6 reentry at runtime 0x0003A27E; Build 0216 runs through the former frame-6030 swarm window to frame 8000)
- **Applicability:** DURABLE VBlank ownership/timing hazard (rastan-direct)
- **Rediscovery Hazard:** HIGH
- **Addresses:** `arcade_pc 0x0003A07A` / `runtime_genesis_pc 0x0003A27A` (`address_map.json` arcade_copy segment) `andi #0xF0FF,SR; rte` -> Build 0216 byte-neutral `ori #0x0600,SR; rte`; `_vblank_service 0x000700C2` tail-jumps to arcade VBlank `0x0003A208`; arcade final RTE at `0x0003A27E`.
- **Source Documents:** docs/design/Cody_build0216_pc090oj_swarm_stack_fix.md; docs/design/Cody_build0216_hurryup_bat_cpu_lock.md; docs/design/Cody_build0216_arcade_bat_path_genesis_lock.md
- **Related Findings:** KF-011 (arcade VBlank owns frame lifecycle), KF-052/KF-053/KF-054 (VBlank budget/timing), KF-047 (PC090OJ retained representation), KF-064/KF-065/KF-067 (lizard-era PC090OJ load)
- **Related Issues:** OPEN-017, OPEN-024
- **Last verified:** 2026-07-19 (Build 0216)

**Finding.** Build 0215's natural hurry-up bat swarm did not expose a local PC090OJ push/pop leak. Static stack audit of `_vblank_service`, `vdp_prepare_sprites`, candidate processing, decode/sync/activate/deactivate/link management, and SAT/tile-DMA helpers found balanced stack paths. The native MAME traces instead show the combined Genesis `_vblank_service` -> arcade VBlank chain becoming long enough that IRQ6 is pending when the arcade VBlank tail reaches `runtime_genesis_pc 0x0003A27A`. The original arcade instruction `andi #0xF0FF,SR` lowers IPM immediately before `rte`; on Genesis this creates a one-instruction reentry window where IRQ6 interrupts at `0x0003A27E`, stacking another frame and entering `_vblank_service` again. Repetition of this pattern explains the falling SP, repeated `0x3A27E` stack material, later impossible PCs, and eventual illegal-instruction/crash-render path. Build 0216 replaces only that 4-byte mask instruction with `ori #0x0600,SR`, keeping IPM >= 6 until the following `rte` restores the original stacked SR. Runtime validation with natural bat-family records (`0x0268/0x0269/0x026A`) reaches frame 8000 with no sampled exception/impossible PC and no `STABLE_SUSPECT_PC` event; the old failure boundary was around frame 6030.

**Use as prior.** Do not debug future overrun-era VBlank stack corruption as a PC090OJ helper push/pop leak without first checking the arcade tail IPM window and reentrant IRQ evidence. The accepted ownership remains `_vblank_service` servicing Genesis hardware and then tail-jumping into arcade VBlank; do not add a second renderer, watchdog recovery, bat suppression, or broad VBlank rewrite. If future VBlank service growth causes timing problems, preserve the final `rte` ownership and avoid lowering IPM before it.

---

## KF-068 — Complete arcade video-hardware surface measured (Ghidra): ~30 leaf translators, zero sprite-RAM readback, two tilemap readbacks; native leaf-boundary re-emission design

- **Status:** ACTIVE (design basis; implementation pending approval)
- **Confidence:** CONFIRMED (Ghidra hw_refs.tsv full catalog + register-indirect completion via linear disasm and runtime lineage KF-044/047/060/065/067)
- **Applicability:** DURABLE — the architectural foundation for the native Genesis video redesign
- **Rediscovery Hazard:** HIGH
- **Addresses:** PC090OJ writers: FUN_0000052A/0x3AD72/0x3B8B0/0x3B902/0x52AA2 (frontend) + 0x41DAE/0x41F5E/0x45DFA (gameplay) + shared composer 0x3D054->0x3C902 (indirect, (A1)+) + ctrl 0x3ADD8/0x3AE28/0x3EEFA/0x3EF5C. PC080SN: frontend/test writers (0x2CA/0x3A4/0x502/0x52A/0x5CC/0x100/0x3AE64/0x3C950/0x3ACAE), gameplay column producer 0x559B2 (indirect, + collision side-channel), scroll committer 0x55AB4 (the game's ONLY gameplay scroll writer, A5+0x10AE/0x10B0/0x10EC/0x10EE), readbacks ONLY at 0x3A552/0x3AC04 (title glyph probes on C-window bytes). Palette: 8 functions (0x264/0x52A/0x3B9F8/0x45D7C/0x45DC4/0x47004/0x59AD4 + source-buffer path A5+0x1600).
- **Source Documents:** docs/design/Andy_fable_native_genesis_video_architecture_redesign.md (hashes of all Ghidra exports and JSON maps inside)
- **Related Findings:** KF-060..067 (the per-defect lineage this design supersedes architecturally), KF-066 (route table survives), KF-067 (map fix relocated to the unified producer)
- **Related Issues:** OPEN-017, OPEN-024
- **Last verified:** 2026-07-20 (hw_refs.tsv 99acceb1..., address_map.json 2fb9a49a...)
...
## KF-072 — N2 plane correctness (Build 0227): the display-off bracket was the sole black-bar cause; the 0226 ring coordinate rewrite was the sole cause of the cyan band + half-screen FG displacement
...
## KF-073 — Stage 1 progression stall blocks the cave; collision producer confirmed KF-067 (row +1) + sparse ground-surface markers (matched arcade vs Genesis)

- **Status:** ACTIVE (investigation; no fix)
- **Confidence:** CONFIRMED (matched arcade 0x10DE00 vs Genesis 0xFF1E00 collision dumps; matched runtime progression)
- **Applicability:** Stage 1 cave/rope/collision; Build 0227 baseline
- **Rediscovery Hazard:** HIGH
- **Source Documents:** docs/design/Andy_fable_build0228_stage1_cave_map_collision_authority.md
- **Related / refines:** KF-067 (collision row-base); KF-041 (cave source model); KF-072 (N2 planes)
