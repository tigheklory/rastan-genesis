...
def decode_frame(maincpu: bytes, family: int, tile_base: int, frame_code: int) -> list[tuple[int, int, int]]:
    table = TABLE_BASES[family]
    off = int.from_bytes(maincpu[table + frame_code * 2 : table + frame_code * 2 + 2], "big")
    ptr = table + off
    parts = []
    while True:
        control = maincpu[ptr]
...
def draw_frame(sprite_region: bytes, parts: list[tuple[int, int, int, int]]) -> tuple[int, int, bytes]:
...
def main() -> int:
    ...
    parser.add_argument("--family", type=int, required=True, choices=sorted(TABLE_BASES))
    parser.add_argument("--tile-base", type=lambda v: int(v, 0), required=True)
    ...
    frames = [args.start + i for i in range(args.count)]
    cell_w = 96
    cell_h = 96
    cols = 4
    ...
    for idx, frame_code in enumerate(frames):
        ...
        parts = decode_frame(maincpu, args.family, args.tile_base, frame_code)
        if parts:
        ...
        marker = (frame_code - args.start) * 4
        for x in range(24):
    ...
    write_rgba_png(args.out, width, height, bytes(image))
