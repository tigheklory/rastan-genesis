...
mkdir -p "${HOMEPATH}/rastanjumptrace"
...
echo "Starting rastanjumptrace — hunting jump to 0xC01CA8"
echo "Output: ${HOMEPATH}/rastanjumptrace/rastan_jump_trace.log"
echo "        ${HOMEPATH}/rastanjumptrace/rastan_jump_summary.txt"
echo ""
...
exec "${MAME_BIN}" "${MACHINE}" \
    ...
    -autoboot_script "${ROOT}/tools/mame/scripts/rastanjumptrace.lua" \
    "$@"
